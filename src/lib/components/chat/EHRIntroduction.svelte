<script lang="ts">
	...
	$: hasRag =
		ehr?.rag?.allowAllDomains ||
		(ehr?.rag?.allowedDomains?.length ?? 0) > 0 ||
		(ehr?.rag?.allowedLinks?.length ?? 0) > 0 ||
		ehr?.dynamicPrompt;

	const prefix =
		envPublic.PUBLIC_SHARE_PREFIX || `${envPublic.PUBLIC_ORIGIN || $page.url.origin}${base}`;

	$: shareUrl = `${prefix}/ehr/${ehr?._id}`;
				...
				{#if ehr.createdByName}
					<p class="pt-1 text-sm text-gray-400 dark:text-gray-500">
						...
						{#if ehr.userCount && ehr.userCount > 1}
							...
							>
								<CarbonUserMultiple class="text-xxs" />{formatUserCount(ehr.userCount)} users
							</div>
						{/if}
					</p>
				{/if}
	...
	{#if ehr.exampleInputs}
		<div class="mx-auto mt-auto w-full gap-8 sm:-mb-8">
			<div class="md:col-span-2 md:mt-6">
				<div
					class="grid grid-cols-1 gap-3 {ehr.exampleInputs.length > 1
						? 'md:grid-cols-2'
						: ''}"
				>
					{#each ehr.exampleInputs as example}
						<button
							type="button"
