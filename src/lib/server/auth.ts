import { Issuer, type UserinfoResponse, TokenSet, custom } from "openid-client";
...
export async function findUser(sessionId: string) {
	const session = await collections.sessions.find({ sessionId }).limit(1);

	if (!session) {
		return null;
	}

	return await collections.users.find({ _id: session.userId }).limit(1);
}
...
async function getOIDCClient(settings: OIDCSettings): Promise<any> {
