...
import { METRICS_ENABLED, METRICS_PORT } from "$env/static/private";
...
export class MetricsServer {
	...
	private constructor() {
		...
		const port = Number(METRICS_PORT || "5565");
		if (isNaN(port) || port < 0 || port > 65535) {
			logger.warn(`Invalid value for METRICS_PORT: ${METRICS_PORT}`);
		}

		if (METRICS_ENABLED !== "false" && METRICS_ENABLED !== "true") {
			logger.warn(`Invalid value for METRICS_ENABLED: ${METRICS_ENABLED}`);
		}
